            ...
            for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
                ...
                if not self.walls[next_x][next_y]:
                    ...
                    if next_position in self.corners:
                        idx = self.corners.index(next_position)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # Step 1: distance from Pacman to the closest unvisited corner
    start_to_corner = min(
        abs(position[0] - x) + abs(position[1] - y)
        for (x, y) in unvisited
    )

    # Step 2: approximate MST cost over unvisited corners
    mst_cost = 0
    remaining = unvisited.copy()

    # Prim-like greedy MST: repeatedly connect nearest corners
    current = remaining.pop(0)
    while remaining:
        dists = []
        for corner in remaining:
            dist = abs(current[0] - corner[0]) + abs(current[1] - corner[1])
            dists.append((dist, corner))
        min_dist, next_corner = min(dists)
        mst_cost += min_dist
        current = next_corner
        remaining.remove(next_corner)

    # Total heuristic = distance to nearest corner + MST over corners
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    position, foodGrid = state
    foodList = foodGrid.asList()  # list of all remaining food positions

    # No food -> goal
    if not foodList:
        return 0

    # Use mazeDistance to compute the true distance to the farthest food
